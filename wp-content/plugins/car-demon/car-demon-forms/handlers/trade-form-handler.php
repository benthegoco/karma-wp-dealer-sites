...
function cd_trade_show_stock() {
	...
	if(isset($_POST['show_stock'])) {
		$show_stock = sanitize_text_field($_POST['show_stock']);
	}
	if ($show_stock==1) {
		if (isset($_POST['stock_num'])) {
			$stock_num = sanitize_text_field($_POST['stock_num']);
		} else {
			$stock_num = '';
		}
	...
	if ($show_stock==2) {
		$car_title = sanitize_text_field($_POST['car_title']);
...
function cd_trade_handler() {
	...
	if ( !wp_verify_nonce( $_REQUEST['nonce'], "cd_contact_us_nonce")) {
		...
		$email_body = apply_filters( 'cd_email_body_filter', $email_body, 'trade' );
		...
		do_action('cd_mail_complete', 'trade', $to, $subject, $email_body, $headers, $_POST['email'], $car_id,$trade_location);
		apply_filters('car_demon_mail_hook_complete', $holder, 'trade', $to, $subject, $email_body, $headers, $_POST['email'], $car_id,$trade_location); //= deprecated
...
function build_trade_body() {
	...
	$trade_location = sanitize_text_field($_POST['trade_location']);
	...
	$selected_car = sanitize_text_field($_POST['selected_car']);
	if (empty($selected_car)) {
	...
	$html = '
		<table align="center" width="450" border="0"'.$style.'>
		  ...
		  <tr>
			...
			$html = apply_filters('cd_form_subscribe_filter', $html, 'trade_form', $trade_location, $selected_car);
			$html = apply_filters('car_demon_mail_hook_subscribe', $html, 'trade_form', $trade_location, $selected_car); //= deprecated
		  $html .= '
		  <tr>
			<td colspan="2"><hr class="hr_margin" /></td>
		  </tr>
...
function get_trade_location($selected_car) {
	...
	if ($posts) {
		foreach ($posts as $post) {
			$post_id = $post->post_id;
			$location_name = get_cd_term( $post_id, 'vehicle_location' );
...
function adfxml_trade($location, $rep_name, $rep_email) {
	...
	if (!empty($selected_car)) {
		...
		$year = get_cd_term( $post_id, 'vehicle_year' );
		$make = get_cd_term( $post_id, 'vehicle_make' );
		$model = get_cd_term( $post_id, 'vehicle_make' );
	...
	//== Contact
	$full_name = sanitize_text_field($_POST['your_name']);
