...
function cd_service_handler() {
	...
	if ( !wp_verify_nonce( $_REQUEST['nonce'], "cd_service_request_nonce")) {
		...
		$email_body = apply_filters( 'cd_email_body_filter', $email_body, 'service_appointment' );
		...
		do_action('cd_mail_complete', 'service_appointment', $to, $subject, $email_body, $headers, $_POST['email'], $post_id, $service_location);
		apply_filters('car_demon_mail_hook_complete', $holder, 'service_appointment', $to, $subject, $email_body, $headers, $_POST['email'], $post_id, $service_location); //= deprecated
...
function send_service_request() {
	...
	$alt_date = sanitize_text_field($_POST['alt_date']);
	$waiting = sanitize_text_field($_POST['waiting']);
	...
	$service_needed = sanitize_text_field($_POST['service_needed']);
	...
	$html = '
		...
		$html = apply_filters('cd_form_subscribe_filter', $html, 'service_appointment', $location, '0');
		$html = apply_filters('car_demon_mail_hook_subscribe', $html, 'service_appointment', $location, '0'); //= deprecated
		  $html .= '<tr>
