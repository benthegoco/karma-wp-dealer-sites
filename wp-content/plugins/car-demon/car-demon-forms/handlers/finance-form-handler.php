...
function send_finance_email() {
	...
	$email_body = apply_filters( 'cd_email_body_filter', $email_body, 'finance' );
	...
	do_action('cd_mail_complete', 'finance', $to, $subject, $email_body, $headers, sanitize_text_field($_POST['ea']), $post_id, $finance_location);
	apply_filters('car_demon_mail_hook_complete', $holder, 'finance', $to, $subject, $email_body, $headers, sanitize_text_field($_POST['ea']), $post_id, $finance_location); //= deprecated
...
function build_finance_body() {
	...
	$x = '
		...
		$x = apply_filters('cd_form_subscribe_filter', $x, 'finance', $location, $selected_car);
		$x = apply_filters('car_demon_mail_hook_subscribe', $x, 'finance', $location, $selected_car); //= deprecated
		  $x .= '
		  <tr>
...
function get_finance_email($finance_location) {
	...
	foreach ($location_list_array as $current_location) {
		if ($finance_location == $current_location) {
