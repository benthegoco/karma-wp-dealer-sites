...
function cd_contact_us_handler() {
	...
	if ( !wp_verify_nonce( $_REQUEST['nonce'], "cd_contact_us_nonce")) {
		...
		$email_body = apply_filters( 'cd_email_body_filter', $email_body, 'contact_us' );
		...
		$post_id = '';
		$holder = '';
		do_action('cd_mail_complete', 'contact_us', $to, $subject, $email_body, $headers, $_POST['email'], $post_id, $_POST['send_to_name']);
		apply_filters('car_demon_mail_hook_complete', $holder, 'contact_us', $to, $subject, $email_body, $headers, $_POST['email'], $post_id, $_POST['send_to_name']); //= deprecated
...
function send_contact_request() {
	...
	$send_to_name = sanitize_text_field($_POST['send_to_name']);
	...
	$html = '
		...
		$html = apply_filters('cd_form_subscribe_filter', $html, 'contact_us', 'unk', '0');
		$html = apply_filters('car_demon_mail_hook_subscribe', $html, 'contact_us', 'unk', '0'); // deprecated
		  $html .= '
		  <tr>
