...
function cd_qualify_handler() {
	...
	if ( !wp_verify_nonce( $_REQUEST['nonce'], "cd_qualify_nonce")) {
		...
		if (isset($car_demon_options['adfxml']) == 'Yes') {
			$semi_rand = md5(time());
		...
		$email_body = apply_filters( 'cd_email_body_filter', $email_body, 'qualify' );
		...
		do_action('cd_mail_complete', 'qualify', $to, $subject, $email_body, $headers, $_POST['email'], $post_id, $_POST['send_to_name']);
		apply_filters('car_demon_mail_hook_complete', $holder, 'qualify', $to, $subject, $email_body, $headers, $_POST['email'], $post_id, $_POST['send_to_name']); //= deprecated
...
function send_qualify_contact_request() {
	...
	$qualify_comment = sanitize_text_field($_POST['qualify_comment']);
	$cd_city = sanitize_text_field($_POST['cd_city']);
	$taa = sanitize_text_field($_POST['taa']);
	$cd_employer = sanitize_text_field($_POST['cd_employer']);
	$cd_income = sanitize_text_field($_POST['cd_income']);
	$taj = sanitize_text_field($_POST['taj']);
	$cd_down = sanitize_text_field($_POST['cd_down']);
	$cd_trade = sanitize_text_field($_POST['cd_trade']);
	$qualify_comment = sanitize_text_field($_POST['qualify_comment']);
	...
	$html = '
		...
		$html = apply_filters('cd_form_subscribe_filter', $html, 'qualify', 'unk', '0');
		$html = apply_filters('car_demon_mail_hook_subscribe', $html, 'qualify', 'unk', '0'); //= deprecated
		  $html .= '
		  <tr>
