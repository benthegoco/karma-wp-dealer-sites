...
function cd_parts_handler() {
	...
	if ( !wp_verify_nonce( $_REQUEST['nonce'], "cd_part_request_nonce")) {
		...
		$email_body = apply_filters( 'cd_email_body_filter', $email_body, 'parts' );
		...
		do_action('cd_mail_complete', 'parts', $to, $subject, $email_body, $headers, $_POST['email'], $post_id, $part_location);
		apply_filters('car_demon_mail_hook_complete', $holder, 'parts', $to, $subject, $email_body, $headers, $_POST['email'], $post_id, $part_location); //= deprecated
...
function send_part_request() {
	...
	$part_needed = sanitize_text_field($_POST['part_needed']);
	...
	$html = '
		...
		$html = apply_filters('cd_form_subscribe_filter', $html, 'parts', $location, '0');
  		$html = apply_filters('car_demon_mail_hook_subscribe', $html, 'parts', $location, '0'); //= deprecated
		  $html .= '<tr>
