...
function cd_service_quote_handler() {
	...
	if ( !wp_verify_nonce( $_REQUEST['nonce'], "cd_service_quote_request_nonce")) {
		...
		$email_body = apply_filters( 'cd_email_body_filter', $email_body, 'service_quote' );
		...
		do_action('cd_mail_complete', 'service_quote', $to, $subject, $email_body, $headers, $_POST['email'], $post_id, $service_location);
...
function send_service_quote_request() {
	...
	$html = '
		...
		$html = apply_filters('cd_form_subscribe_filter', $html, 'service_quote', $location, '0');
		$html = apply_filters('car_demon_mail_hook_subscribe', $html, 'service_quote', $location, '0'); //= deprecated
		  $html .= '<tr>
