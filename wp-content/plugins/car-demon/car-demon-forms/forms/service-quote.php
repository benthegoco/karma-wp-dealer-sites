...
function car_demon_service_quote($location, $popup_id = '', $popup_button='') {
	...
	if (isset($car_demon_options['use_form_css'])) {
		if ($car_demon_options['use_form_css'] != 'No') {
			wp_enqueue_style('car-demon-service-quote-css', plugins_url() . '/car-demon/car-demon-forms/forms/css/car-demon-service-quote.css', array(), CAR_DEMON_VER);
		}
	} else {
		wp_enqueue_style('car-demon-service-quote-css', plugins_url() . '/car-demon/car-demon-forms/forms/css/car-demon-service-quote.css', array(), CAR_DEMON_VER);
	}
	...
	if (!empty($popup_id)) {
		...
		wp_enqueue_script('car-demon-service-quote-popup-js', plugins_url() . '/car-demon/car-demon-forms/forms/js/car-demon-service-quote-popup.js', array(), CAR_DEMON_VER);
		if (isset($car_demon_options['use_form_css'])) {
			if ($car_demon_options['use_form_css'] != 'No') {
				wp_enqueue_style('car-demon-service-quote-popup-css', plugins_url() . '/car-demon/car-demon-forms/forms/css/car-demon-service-quote-popup.css', array(), CAR_DEMON_VER);
			}
		} else {
			wp_enqueue_style('car-demon-service-quote-popup-css', plugins_url() . '/car-demon/car-demon-forms/forms/css/car-demon-service-quote-popup.css', array(), CAR_DEMON_VER);
		}
	...
	$x .= '
			...
			$x = apply_filters('cd_form_filter', $x, 'service_quote', '');
			$x = apply_filters('car_demon_mail_hook_form', $x, 'service_quote', 'unk'); //= deprecated
