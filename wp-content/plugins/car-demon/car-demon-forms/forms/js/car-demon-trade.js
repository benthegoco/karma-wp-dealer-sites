...
function car_demon_validate_trade() {
	var form_id = "";
	var msg = "";
	var name_valid = 0;

	var html = '<img src="' + cdTradeParams.spinner + '" /> ' + cdTradeParams.sending_msg;
	document.getElementById("trade_msg"+form_id).style.display = "block";
	document.getElementById("trade_msg"+form_id).style.background = "#FFFFFF";
	document.getElementById("trade_msg"+form_id).innerHTML = html;
...
//				cd_valid("voi_title","trade_form"+form_id);
	...
	if (msg != "") {
		...
		var send_data = {action: 'cd_trade_handler', 'nonce': nonce, 'your_name': your_name,'phone':phone, 'email':email, 'trade_location':trade_location, 'year':year, 'make':make, 'model':model, 'miles':miles, 'vin':vin, 'comment':comment, 'options':options, 'selected_car':selected_car};

		eval(cdTradeParams.form_data);
		...
		jQuery.ajax({
			...
			success: function(html) {
				...
				document.getElementById("trade_msg"+form_id).style.background = "#FFFFFF";
				document.getElementById("trade_msg"+form_id).innerHTML = html;
				document.getElementById("trade_form"+form_id).style.display = "none";
