...
function car_demon_validate_service_quote(form_id) {
	...
	var html = '<img src="' + cdServiceQuoteParams.spinner + '" /> ' + cdServiceQuoteParams.sending_msg;
	...
	eval(cdServiceQuoteParams.form_js);
	...
	if (msg != "") {
		...
		var send_data = {action: 'cd_service_quote_handler', 'nonce': nonce, 'your_name': your_name,'phone':phone, 'email':email, 'service_location':service_location, 'year':year, 'make':make, 'model':model, 'miles':miles, 'vin':vin, 'service_needed':service_needed};

		eval(cdServiceQuoteParams.form_data);
