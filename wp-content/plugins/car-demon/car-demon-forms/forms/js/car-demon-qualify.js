...
function car_demon_validate_qualify(form_id) {
	...
	var html = '<img src="' + cdQualifyParams.spinner + '" /> ' + cdQualifyParams.sending_msg;
	...
	if (msg != "") {
		...
		var send_data = {action: 'cd_qualify_handler', 'nonce': nonce, 'your_name': your_name, 'phone':phone, 'email':email, 'send_to':send_to, 'send_to_name':send_to_name, 'qualify_comment':qualify_comment, 'cd_city':cd_city, 'taa':taa, 'cd_employer':cd_employer, 'cd_income':cd_income, 'taj':taj, 'cd_down':cd_down, 'cd_trade':cd_trade};
		
		eval(cdQualifyParams.form_data);
