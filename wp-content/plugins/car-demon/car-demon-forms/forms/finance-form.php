...
//		}
	...
	if ($car_demon_options['secure_finance'] == 'Yes') {
		...
		<form name="frm_app" id="frm_app" action="CreditApp_ex.asp" method="post" class="cdform finance_form">
			...
			<div class="finance_segment_wide">
				<div align="center">
					...
					<br /><?php echo $bad; ?><br />
					<?php $fin = apply_filters('cd_form_filter', $fin, 'finance', '');
					$fin = apply_filters('car_demon_mail_hook_form', $fin, 'finance', 'unk'); // deprecated
					echo $fin;
					?>
					...
					<img class="cd_secure" src="<?php echo $car_demon_pluginpath; ?>theme-files/images/secure.gif" alt="Secure Encryption" /></p>
				</div>
			</div>
		</form>
	</div>
<?php
	if ( !empty($bad) ) {
		echo '
			<script>
				...
				var finance_location = document.frm_app.finance_location[0];
				if (finance_location != null) {
	...
	ob_end_clean();
	$output = apply_filters('car_demon_finance_filter', $output ); //= deprecated
	$output = apply_filters('cd_finance_filter', $output );
	return $output;
}
...
function get_finance_for_vehicle($stock_num) {
	...
	if ($posts) {
		foreach ($posts as $post) {
			...
			$vehicle_photo = cd_main_photo( $post_id, 'thumbnail' );
		}
	}
	$x = '
