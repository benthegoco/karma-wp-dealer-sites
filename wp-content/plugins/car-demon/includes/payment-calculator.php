...
function car_demon_calculator_form( $price, $apr, $term, $disclaimer1, $disclaimer2 ) {
	...
	wp_enqueue_script( 'car-demon-payment-calculator-js', plugins_url() . '/car-demon/widgets/js/car-demon-calculator-widget.js', array(), CAR_DEMON_VER );
	if ( isset( $car_demon_options['use_form_css'] ) ) {
		if ( $car_demon_options['use_form_css'] != 'No' ) {
			wp_enqueue_style('car-demon-payment-calculator-css', plugins_url() . '/car-demon/widgets/css/car-demon-calculator-widget.css', array(), CAR_DEMON_VER);
		}
	}
	?>
