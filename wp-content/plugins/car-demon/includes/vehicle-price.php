...
function get_vehicle_price( $post_id ) {
	...
	if ( $show_price == __( 'Yes', 'car-demon' ) ) {
		...
		if ( $vehicle_condition == __( 'New', 'car-demon' ) ) {
			$price .= '<p>&nbsp;</p><div class="your_price no_price">' . get_option( $vehicle_location_slug . '_no_new_price' ) . '</div>';
		} else {
			$price .= '<p>&nbsp;</p><div class="your_price no_price">' . get_option( $vehicle_location_slug . '_no_used_price' ) . '</div>';
		}
	}
