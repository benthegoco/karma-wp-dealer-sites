...
function get_car_contact( $post_id ) {
	...
	$car_contact['sales_type'] = $sales_type;
	...
	if ( $current_location_terms ) {
		...
		$current_location = cd_get_default_location_slug();
	}
	...
	if ( $sales_code ) {
		...
		if ( $user_finance == 1 ) {
			...
			if ( ! empty( $user_phone ) ) {
				...
				$car_contact['finance_phone'] = get_option( $current_location . '_finance_number' );
			}
		} else {
		...
		$car_contact['sales_email'] = get_option( $current_location . '_' . $sales_type. '_sales_email' );
		$car_contact['sales_phone'] = get_option( $current_location . '_' . $sales_type. '_sales_number' );
		$car_contact['sales_mobile'] = get_option( $current_location . '_' . $sales_type. '_mobile_number' );
		$car_contact['sales_mobile_provider'] = get_option( $current_location . '_' . $sales_type. '_mobile_provider' );
		$car_contact['trade_name'] = get_option( $current_location . '_trade_name' );
		$car_contact['trade_email'] = get_option( $current_location . '_trade_email' );
		$car_contact['trade_phone'] = get_option( $current_location . '_trade_number' );
...
function cd_get_default_location_slug() {
	if ( defined( 'CD_DEFAULT_LOCATION_SLUG' ) ) {
		return CD_DEFAULT_LOCATION_SLUG;
	}

	$terms = get_terms( 'vehicle_location', array(
		'hide_empty' => false,
		'orderby' => 'term_id',
	) );
...
function cd_get_default_location_name() {
	if ( defined( 'CD_DEFAULT_LOCATION_NAME' ) ) {
		return CD_DEFAULT_LOCATION_NAME;
	}
