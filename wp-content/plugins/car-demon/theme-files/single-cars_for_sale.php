...
if (isset($car_demon_options['use_vehicle_css'])) {
	if ($car_demon_options['use_vehicle_css'] != 'No') {
		...
		wp_enqueue_style('car-demon-single-car-css', plugins_url() . '/car-demon/theme-files/css/car-demon-single-car.css', array(), CAR_DEMON_VER);
	}
} else {
	wp_enqueue_style('car-demon-vin-query-css', plugins_url() . '/car-demon/vin-query/css/car-demon-vin-query.css', array(), CAR_DEMON_VER);
	wp_enqueue_style('car-demon-single-car-css', plugins_url() . '/car-demon/theme-files/css/car-demon-single-car.css', array(), CAR_DEMON_VER);
}
wp_register_script('car-demon-single-car-js', plugins_url() . '/car-demon/theme-files/js/car-demon-single-cars.js', array(), CAR_DEMON_VER);
...
do_action( 'cd_header_sidebar_action' );
	if ( have_posts() ) while ( have_posts() ) : the_post();
		$post_id = get_the_ID();
		$html = apply_filters('cd_vdp_filter', car_demon_single_car($post_id), $post_id );
		$html = apply_filters('car_demon_single_car_filter', $html, $post_id ); //= deprecated
		echo $html;
