...
function cd_single_vehicle_content( $post_id, $content ) {
	...
	$x = apply_filters( 'cd_vdp_filter', $output, $post_id );
	$x = apply_filters( 'car_demon_single_car_filter', $x, $post_id ); //= deprecated
	return $x;
}
...
function cd_loop_vehicle_content( $post_id, $content ) {
	...
	//	if( $current_post == 0 ) { 
			/*first post*/
	//	}
	$x = '<div class="car_item">' . cdcr_loop( '', $post_id ) . '</div>';
	return $x;
}
