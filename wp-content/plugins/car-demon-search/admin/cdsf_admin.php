<?php
function cdsf_settings_page() {
	add_submenu_page( 'edit.php?post_type=cars_for_sale', 'Pro Search', 'Pro Search', 'edit_pages', 'cdsf_options', 'cdsf_options_do_page' );
}
add_action( 'admin_menu', 'cdsf_settings_page' );

function cdsf_options_do_page() {
	$month = date("m");
	$day = date("d");
	$year = date("Y");
	$sec = "0";
	...
	$x = '<div class="wrap cd_welcome">';
		...
		if (isset($_POST['build_cache_now'])) {
			...
			//print_r(cdsf_get_current_inventory());
		}
		cdsf_set_schedule();
				...
				$x .= '<ul>';
					$x .= '<li>';
						$atts = array(
							...
							'value_location' => '"" //= value_ fields will set the selected value when the form loads',
							'value_condition' => '""',
							'value_year' => '""',
							'value_make' => '""',
							'value_model' => '""',
							...
							'style' => 1 .' //= style to load for this form [exp. 1, 2, 3 or 4] blank for none',
		...
		$x .= '<div class="cd_welcome_tab" id="cd_settings">
				...
				$x .=  '<form action="" method="post">';
					...
					$x .=  '</select>';
					$x .=  '<input type="hidden" name="set_cdsf_schedule" value="1" />';
					$x .=  '<input type="submit" name="submit" value="'.__('Set json build time', 'car-demon-search').'" />';
				$x .=  '</form>';
				$timestamp = wp_next_scheduled('build_cdsf_cache');
				if (!empty($timestamp)) {
				...
				if (isset($_POST['cdsf_use_css_form'])) {
					update_option('cdsf_use_css_form', $_POST['cdsf_use_css_form_val']);
				}
				...
				$x .=  '<blockquote>';
					$x .=  '<form action="" method="post" enctype="multipart/form-data">';
						$x .=  '<input type="hidden" name="cdsf_use_css_form" value="1" /> ';
						$x .=  '<select id="cdsf_use_css_form_val" name="cdsf_use_css_form_val">';
				...
				if (isset($_POST['cdsf_use_css_jqueryui'])) {
					update_option('cdsf_use_css_jqueryui', $_POST['cdsf_use_css_jqueryui_val']);
				}
				...
				$x .=  '<blockquote>';
					$x .=  '<form action="" method="post" enctype="multipart/form-data">';
						$x .=  '<input type="hidden" name="cdsf_use_css_jqueryui" value="1" /> ';
						$x .=  '<select id="cdsf_use_css_jqueryui_val" name="cdsf_use_css_jqueryui_val">';
				...
				//= Load CSS for jqueryui
				if (isset($_POST['cdsf_hide_count'])) {
					update_option('cdsf_hide_count', $_POST['cdsf_hide_count_val']);
				}
				...
				$x .=  '<h3>'.__('VI. Hide Item Counts:', 'car-demon-search').'</h3>';
				$x .=  '<blockquote>';
					$x .=  '<form action="" method="post" enctype="multipart/form-data">';
						$x .=  '<input type="hidden" name="cdsf_hide_count" value="1" /> ';
						$x .=  '<select id="cdsf_hide_count_val" name="cdsf_hide_count_val">';
							$x .=  '<option value="on"'.$select_yes.'>'.__('Yes', 'car-demon-search').'</option>';
							$x .=  '<option value="off"'.$select_no.'>'.__('No', 'car-demon-search').'</option>';
						$x .=  '</select>';
						$x .=  '<input type="submit" name="submit" value="'.__('Save', 'car-demon-search').'" />';
					$x .=  '</form>';
				$x .=  '</blockquote>';
		...
		$x .= '</div>';
//		$x .= cdsf_color_control();
	$x .= '</div>';
	echo $x;
}

function cdsf_set_schedule() {
	if (isset($_POST['set_cdsf_schedule'])) {
		$month = date("m");
		$day = date("d");
		$year = date("Y");
		$sec = "0";
		...
		wp_clear_scheduled_hook('build_cdsf_cache');	
		wp_schedule_event($new_time, 'daily', 'build_cdsf_cache');
	}
	if (isset($_POST['remove_schedule'])) {
		wp_clear_scheduled_hook('build_cdsf_cache');	
	}
}
function cdsf_schedule() {
	cdsf_delete_cache();
	cdsf_build_cache();
}
add_action( 'build_cdsf_cache', 'cdsf_schedule' );
?>